...
using EVM.Services.Features.EventTask.Models.Responses;
...
namespace EVM.Services.Features.EventTask.Query;

public class GetByIdEventTaskQueryHandler
    (ILogger<GetByIdEventTaskQueryHandler> _logger, AppDbContext _appDbContext, IHttpContextAccessor _httpContextAccessor, IAuthorizationService _authorizationService)
{
    private readonly HttpContext _httpContext = _httpContextAccessor.HttpContext ?? throw new MissingHttpContextException();

    public async Task<ApiResponse<GetEventTaskResponse>> Handle(Guid taskId, CancellationToken cancellationToken)
    {
        ...
        var task = await _appDbContext.EventTasks.Where(x => x.UserId == userId && x.Id == taskId)
            .Select(x => new GetEventTaskResponse
            {
                TaskId = taskId,
                Title = x.Title,
